...
${Green_font_prefix} 4. ${Font_color_suffix}本机到指定 IPv6 路由 英文 输出 Nexttrace库（可指定端口）
    "
    stty erase '^H' && read -p " 请输入数字 [1-4] (默认: 取消):" Specify_IP_num < /dev/tty
...
#IPv4输入模块 IP检查
Int_IPV4(){
    read -e -p "请输入目标 IPv4：" Int_IPV4_IP < /dev/tty
...
Int_IPV4_P(){
    read -e -p "请输入指定的端口（默认 80）：" Int_IPV4_Prot < /dev/tty
...
#IPv6输入模块
Int_IPV6(){
    read -e -p "请输入目标 IPv6：" Int_IPV6_IP < /dev/tty
     [[ -z "${Int_IPV6_IP}" ]] && echo -e "${Error} 未输入 IP，已退出" && exit 1 
    #检查IP
    Check_Int_IPV6
    read -e -p "请输入指定的端口（默认 80）：" Int_IPV6_Prot < /dev/tty
...
Check_Int_IPV4(){
    ...
    if [[ -z "${PING_IPV4_CHECK}" ]]; then
        ...
        stty erase '^H' && read -p "(默认: y):" unyn < /dev/tty
        if [[ ${unyn} == [Nn] ]]; then
...
—————————————————————————————————————————————————————————————————————
 ...
 6. 退出测试
    ...
    " 
    read -e -p " 请输入需要的测试项 [1-6] ( 默认：1 ）：" Stand_AutoTrace_num < /dev/tty
