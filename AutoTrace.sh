...
print_route_summary() {
    ...
    for item in "${ROUTE_SUMMARY[@]}"; do
        ...
        echo "$target" | grep -qE "联通" && tgt_color="$Cyan"
        echo "$target" | grep -qE "移动" && tgt_color="$Blue"

        # 线路着色（根据回程线路）—— 运营商同上，其他类别用洋红/白/红
        local rt_color="$White"
        echo "$route" | grep -qE "电信" && rt_color="$Yellow"
        echo "$route" | grep -qE "联通" && rt_color="$Cyan"
        ...
        echo "$route" | grep -qiE "PCCW|HGC|HKBN|HKT|HKIX" && rt_color="$Magenta"
        echo "$route" | grep -qiE "HiNet|Seednet|远传|TWM|FETnet|APTG|TBC" && rt_color="$Magenta"
        echo "$route" | grep -qiE "韩国|KINX" && rt_color="$Magenta"
        echo "$route" | grep -qiE "Singtel|StarHub|MyRepublic|Telstra|Optus|TPG|Vocus|Spark|越南|泰国|菲律宾|印尼|新加坡|澳洲|新西兰" && rt_color="$Magenta"
        echo "$route" | grep -qiE "印度|Jio|Airtel|BSNL|VSNL|MTNL" && rt_color="$Magenta"
        ...
        echo "$route" | grep -qiE "Rostelecom|TTK|STC|Etisalat|du |Zain|Ooredoo|Bezeq|Egypt|MTN|Liquid|Safaricom|SEACOM|Vodacom|Maroc|Telmex|Claro|Vivo|Oi |Embratel|TIM" && rt_color="$Magenta"
        echo "$route" | grep -q "未识别" && rt_color="$White"

        # 延迟着色 —— 绿/洋红/红（与运营商颜色完全不重叠）
        local lat_color="$Green"
        if [ "$latency" != "-" ]; then
            local lat_int=${latency%%.*}
            [ "$lat_int" -gt 200 ] 2>/dev/null && lat_color="$Red"
            [ "$lat_int" -le 200 ] 2>/dev/null && [ "$lat_int" -gt 150 ] 2>/dev/null && lat_color="$Magenta"
    ...
    echo -e "${Bold}└────────┴──────────────┴──────────────────────┴──────────────┘${Reset}" | tee -a $log
    echo -e "${Dim}颜色说明: ${Yellow}电信${Reset} ${Cyan}联通${Reset} ${Blue}移动${Reset} ${Magenta}海外/云CDN${Reset} ${Red}骨干/IX${Reset}  ${Dim}延迟: ${Green}<150ms${Reset} ${Magenta}150-200ms${Reset} ${Red}>200ms${Reset}" | tee -a $log
    echo "" | tee -a $log
    ROUTE_SUMMARY=()
}
# ======= 线路判断函数结束 =======
...
check_sys(){
    ...
    [[ -z "$(command -v wget)" ]] && echo -e "${Info} 开始安装 wget ...." && _install_pkg wget
    [[ -z "$(command -v curl)" ]] && echo -e "${Info} 开始安装 curl ...." && _install_pkg curl
    [[ -z "$(command -v ping)" ]] && echo -e "${Info} 开始安装 iputils-ping ...." && _install_pkg iputils-ping
}
...
checkver() {
    ...
    [[ -z "$remote_ver" ]] && return
    [[ "$remote_ver" == "$sh_ver" ]] && return
    # Different version - auto update
    echo -e "${Info} 发现新版本 v${remote_ver}，正在自动更新..."
...
map_ip_type() {
    local type_raw="$1"
    case "$type_raw" in
        "Data Center/Web Hosting/Transit") echo "数据中心" ;;
        "Fixed Line ISP") echo "家庭宽带" ;;
        "Commercial") echo "商业宽带" ;;
        "Mobile ISP") echo "移动流量" ;;
        "Content Delivery Network") echo "内容分发网络(CDN)" ;;
        "Search Engine Spider") echo "搜索引擎蜘蛛" ;;
        "University/College/School") echo "教育网" ;;
        *) echo "未知 IP 网络类型" ;;
    esac
}
...
IP_Check(){
    ...
    # 提前下载 curl-impersonate（如果不是 mips）
    if [[ ${bit} != "mips" ]]; then
    ...
    #如果IPv4不为空，就执行里面的
    if [ -n "$WAN_4" ]; then
      #缓存 API JSON 响应，减少重复请求
      IP_4_JSON=$(curl -s4m5 "http://ip-api.com/json/$WAN_4")
      #输出IP的ISP
      ISP_4=$(echo "$IP_4_JSON" | grep -Po '"isp":\s*"\K[^"]*')
      #输出IP的ASN
      ASN_4=$(echo "$IP_4_JSON" | grep -Po '"as":\s*"\K[^"]*')
      #输出IP的服务商
      Host_4=$(echo "$IP_4_JSON" | grep -Po '"org":\s*"\K[^"]*')
      #输出IP的国家，英文
      COUNTRY_4E=$(echo "$IP_4_JSON" | grep -Po '"country":\s*"\K[^"]*')
      #输出IP的地址，英文
      City_4E=$(echo "$IP_4_JSON" | grep -Po '"city":\s*"\K[^"]*')
      Region_4E=$(echo "$IP_4_JSON" | grep -Po '"regionName":\s*"\K[^"]*')
      Region_code_4E=$(echo "$IP_4_JSON" | grep -Po '"region":\s*"\K[^"]*')
      ...
      TYPE_4=$(map_ip_type "$TYPE_4_Temp")
    fi  
    ...
    if [ -n "$WAN_6" ]; then
      #缓存 API JSON 响应，减少重复请求
      IP_6_JSON=$(curl -s6m5 "http://ip-api.com/json/$WAN_6")
      #输出IP的ISP
      ISP_6=$(echo "$IP_6_JSON" | grep -Po '"isp":\s*"\K[^"]*')
      #输出IP的ASN
      ASN_6=$(echo "$IP_6_JSON" | grep -Po '"as":\s*"\K[^"]*')
      #输出IP的服务商
      Host_6=$(echo "$IP_6_JSON" | grep -Po '"org":\s*"\K[^"]*')
      #输出IP的国家，英文
      COUNTRY_6E=$(echo "$IP_6_JSON" | grep -Po '"country":\s*"\K[^"]*')
      #输出IP的地址，英文
      City_6E=$(echo "$IP_6_JSON" | grep -Po '"city":\s*"\K[^"]*')
      Region_6E=$(echo "$IP_6_JSON" | grep -Po '"regionName":\s*"\K[^"]*')
      Region_code_6E=$(echo "$IP_6_JSON" | grep -Po '"region":\s*"\K[^"]*')
      ...
      -H "Key: c97ab9480e282182aeac0408b788fad9e41d3ef5aa12d294b3fe8b50cfeb4edf43351bbe4870b066" \
      -H "Accept: application/json" | grep -Po '"usageType": *\K"[^"]*"' | sed "s#\\\##g" | sed 's/"//g')
      TYPE_6=$(map_ip_type "$TYPE_6_Temp")
    fi

    # 统一删除 curl-impersonate
    Curl_impersonate_Dle
...
#Nexttrace 合并 IPv4/IPv6 回程代码 中文输出 
NT_mtr_CN(){
    ...
    if [ "$2" = "tcp" ] || [ "$2" = "TCP" ]; then
        ...
        echo -e "\n$5 Tracecroute to $4 (ICMP Mode, Max $3 Hop, $ip_ver)" | tee -a $log
        echo -e "===================================================================" | tee -a $log
...
NT_IPv4_IP_CN_Mtr(){
    ...
    #开始测试IPv4库回程路由，第5个块是表示节点序号的，增删节点都要修改
 	NT_mtr_CN "${IPv4_1}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_1_name}" "No:1/9" "IPv4"
    judge_route "$log" "${IPv4_1_name}" "No:1/9"
    NT_mtr_CN "${IPv4_2}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_2_name}" "No:2/9" "IPv4"
    judge_route "$log" "${IPv4_2_name}" "No:2/9"
    NT_mtr_CN "${IPv4_3}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_3_name}" "No:3/9" "IPv4"
    judge_route "$log" "${IPv4_3_name}" "No:3/9"
    NT_mtr_CN "${IPv4_4}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_4_name}" "No:4/9" "IPv4"
    judge_route "$log" "${IPv4_4_name}" "No:4/9"
    NT_mtr_CN "${IPv4_5}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_5_name}" "No:5/9" "IPv4"
    judge_route "$log" "${IPv4_5_name}" "No:5/9"
    NT_mtr_CN "${IPv4_6}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_6_name}" "No:6/9" "IPv4"
    judge_route "$log" "${IPv4_6_name}" "No:6/9"
    NT_mtr_CN "${IPv4_7}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_7_name}" "No:7/9" "IPv4"
    judge_route "$log" "${IPv4_7_name}" "No:7/9"
    NT_mtr_CN "${IPv4_8}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_8_name}" "No:8/9" "IPv4"
    judge_route "$log" "${IPv4_8_name}" "No:8/9"
    NT_mtr_CN "${IPv4_9}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_9_name}" "No:9/9" "IPv4"
...
NT_IPv4_IP_EN_Mtr(){
    ...
    nexttrace_detect_pow
    #开始测试IPv4库回程路由，第5个块是表示节点序号的，增删节点都要修改
 	NT_mtr_EN "${IPv4_1}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_1_name}" "No:1/9" "IPv4"
    judge_route "$log" "${IPv4_1_name}" "No:1/9"
    NT_mtr_EN "${IPv4_2}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_2_name}" "No:2/9" "IPv4"
    judge_route "$log" "${IPv4_2_name}" "No:2/9"
    NT_mtr_EN "${IPv4_3}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_3_name}" "No:3/9" "IPv4"
    judge_route "$log" "${IPv4_3_name}" "No:3/9"
    NT_mtr_EN "${IPv4_4}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_4_name}" "No:4/9" "IPv4"
    judge_route "$log" "${IPv4_4_name}" "No:4/9"
    NT_mtr_EN "${IPv4_5}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_5_name}" "No:5/9" "IPv4"
    judge_route "$log" "${IPv4_5_name}" "No:5/9"
    NT_mtr_EN "${IPv4_6}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_6_name}" "No:6/9" "IPv4"
    judge_route "$log" "${IPv4_6_name}" "No:6/9"
    NT_mtr_EN "${IPv4_7}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_7_name}" "No:7/9" "IPv4"
    judge_route "$log" "${IPv4_7_name}" "No:7/9"
    NT_mtr_EN "${IPv4_8}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_8_name}" "No:8/9" "IPv4"
    judge_route "$log" "${IPv4_8_name}" "No:8/9"
    NT_mtr_EN "${IPv4_9}" "${Net_Mode}" "${Hop_Mode}" "${IPv4_9_name}" "No:9/9" "IPv4"
    judge_route "$log" "${IPv4_9_name}" "No:9/9"
    #输出线路判断汇总
    print_route_summary "$log"
    #保留IPv4回程路由日志
...
NT_IPv6_IP_CN_Mtr(){
    ...
    #开始测试IPv6库回程路由，第5个块是表示节点序号的，增删节点都要修改
 	NT_mtr_CN "${IPv6_1}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_1_name}" "No:1/9" "IPv6"
    judge_route "$log" "${IPv6_1_name}" "No:1/9"
    NT_mtr_CN "${IPv6_2}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_2_name}" "No:2/9" "IPv6"
    judge_route "$log" "${IPv6_2_name}" "No:2/9"
    NT_mtr_CN "${IPv6_3}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_3_name}" "No:3/9" "IPv6"
    judge_route "$log" "${IPv6_3_name}" "No:3/9"
    NT_mtr_CN "${IPv6_4}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_4_name}" "No:4/9" "IPv6"
    judge_route "$log" "${IPv6_4_name}" "No:4/9"
    NT_mtr_CN "${IPv6_5}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_5_name}" "No:5/9" "IPv6"
    judge_route "$log" "${IPv6_5_name}" "No:5/9"
    NT_mtr_CN "${IPv6_6}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_6_name}" "No:6/9" "IPv6"
    judge_route "$log" "${IPv6_6_name}" "No:6/9"
    NT_mtr_CN "${IPv6_7}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_7_name}" "No:7/9" "IPv6"
    judge_route "$log" "${IPv6_7_name}" "No:7/9"
    NT_mtr_CN "${IPv6_8}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_8_name}" "No:8/9" "IPv6"
    judge_route "$log" "${IPv6_8_name}" "No:8/9"
    NT_mtr_CN "${IPv6_9}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_9_name}" "No:9/9" "IPv6"
...
NT_IPv6_IP_EN_Mtr(){
    ...
    nexttrace_detect_pow
    #开始测试IPv6库回程路由，第5个块是表示节点序号的，增删节点都要修改
 	NT_mtr_EN "${IPv6_1}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_1_name}" "No:1/9" "IPv6"
    judge_route "$log" "${IPv6_1_name}" "No:1/9"
    NT_mtr_EN "${IPv6_2}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_2_name}" "No:2/9" "IPv6"
    judge_route "$log" "${IPv6_2_name}" "No:2/9"
    NT_mtr_EN "${IPv6_3}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_3_name}" "No:3/9" "IPv6"
    judge_route "$log" "${IPv6_3_name}" "No:3/9"
    NT_mtr_EN "${IPv6_4}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_4_name}" "No:4/9" "IPv6"
    judge_route "$log" "${IPv6_4_name}" "No:4/9"
    NT_mtr_EN "${IPv6_5}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_5_name}" "No:5/9" "IPv6"
    judge_route "$log" "${IPv6_5_name}" "No:5/9"
    NT_mtr_EN "${IPv6_6}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_6_name}" "No:6/9" "IPv6"
    judge_route "$log" "${IPv6_6_name}" "No:6/9"
    NT_mtr_EN "${IPv6_7}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_7_name}" "No:7/9" "IPv6"
    judge_route "$log" "${IPv6_7_name}" "No:7/9"
    NT_mtr_EN "${IPv6_8}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_8_name}" "No:8/9" "IPv6"
    judge_route "$log" "${IPv6_8_name}" "No:8/9"
    NT_mtr_EN "${IPv6_9}" "${Net_Mode}" "${Hop_Mode}" "${IPv6_9_name}" "No:9/9" "IPv6"
    judge_route "$log" "${IPv6_9_name}" "No:9/9"
    #输出线路判断汇总
    print_route_summary "$log"
    #保留IPv6回程路由日志
...
Nexttrace_Ver(){
    Nexttrace_bit
    echo -e "${Info} Nexttrace 已下载 !" | tee -a $log
}

#Nexttrace 系统位数版本下载（含缓存检测）
Nexttrace_bit(){
    #通过Github API获取最新版本号
    ...
    if [[ -e ${Nexttrace_file} ]]; then
        local cur_ver=$(${Nexttrace_file} --version 2>&1 | grep -oP '[0-9]+\.[0-9]+\.[0-9]+' | head -1)
        if [[ "$cur_ver" == "$NT_Ver" ]]; then
            echo -e "${Info} Nexttrace 已是最新版本 ($NT_Ver)，跳过下载"
    ...
    echo -e "${Info} Nexttrace 下载完成 !" | tee -a $log
}

#Curl-impersonate版本下载
Curl_impersonate_Ver(){
    Curl_impersonate_bit
    echo -e "${Info} Curl-impersonate 已下载 !" | tee -a $log
}
